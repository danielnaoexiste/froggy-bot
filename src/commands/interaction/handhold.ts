...
const command: SlashCommand = {
  command: new SlashCommandBuilder()
    .setName("handhold")
    .setDescription("Holds hands with a target!") 
  ...
  execute: async interaction => {
    ...
    const actionDescription = "holds hands with"
     
    const gif = await fetch(`${process.env.OTAKU_GIFS}handhold`).then(res => res.json())
