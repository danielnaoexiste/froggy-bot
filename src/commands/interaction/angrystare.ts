...
const command: SlashCommand = {
  command: new SlashCommandBuilder()
    .setName("angrystare")
    .setDescription("Stares angrily at a target!") 
  ...
  execute: async interaction => {
    ...
    const actionDescription = "stares angrily at"
     
    const gif = await fetch(`${process.env.OTAKU_GIFS}angrystare`).then(res => res.json())
